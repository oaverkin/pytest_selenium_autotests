...
class TestLogin:
    ...
    def test_login_with_wrong_password(self):
        self.login_page.open(url)
        self.login_page.login_with_credentials(settings.get("stage").login, "password")
        assert self.login_page.get_title() == login_title
    ...
    def test_login_with_wrong_username(self):
        self.login_page.login_with_credentials("uesername@email.com", settings.get("stage").password)
        assert self.login_page.get_title() == login_title
        assert self.login_page.get_error_message("Invalid email or password") is True
